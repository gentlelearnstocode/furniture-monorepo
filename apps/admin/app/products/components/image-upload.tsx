  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
        {value.map((image) => (
          <div key={image.assetId} className="relative aspect-square rounded-md overflow-hidden border bg-gray-50 flex items-center justify-center group">
            <Image
              src={image.url}
              alt="Product Image"
              fill
...
function InputFile({ onUpload, disabled }: { onUpload: (e: React.ChangeEvent<HTMLInputElement>) => void, disabled?: boolean }) {
    return (
        <input
            type="file"
            multiple
